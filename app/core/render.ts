...
export class RenderJob {
  ...
  start(ctx: ToolContext) {
    ...
    return new Promise<void>((accept, reject) => {
      let this2 = this;
      function* loop(timerid: number): Generator<void, void, void> {
      ...
      let job: Generator<void, void, void> | undefined;

      let timerid = window.setInterval(() => {
        if (!job) {
          job = loop(timerid)[Symbol.iterator]();
        }
  ...
  * renderTileIntern(tile: RenderTile) {
    let ctx = this.ctx;
    if (!ctx) {
      throw new Error("render job has no context");
    }
    let editor = ctx.canvas;
    let pattern = this.pattern;
    let gl = editor.gl;
    if (!gl || !editor.canvas) {
      throw new Error("canvas editor has no gl context");
    }
    let size = this.size;
    let sizeVec = new Vector2(size);
    ...
    uv2.add(new Vector2([tile.width, tile.height]));
    uv2.div(sizeVec);
    ...
    for (let i = 0; i < pattern.max_samples; i++) {
      ...
      pattern._doViewportDraw(ctx, editor.canvas, gl, true,
        false, null, [[uv1[0], uv1[1]], [uv2[0], uv2[1]]], [this.size[0], this.size[1]]);
    ...
    let fbo = this.fbo;
    if (!fbo) {
      throw new Error("render job has no fbo");
    }
    ...
    gl.bindFramebuffer(gl.READ_FRAMEBUFFER, fbo.fbo ?? null);
...
export var renderJob: RenderJob | undefined = undefined;

export function render(ctx: ToolContext, width: number, height: number, tilesize = 512) {
