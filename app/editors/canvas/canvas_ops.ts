...
import type {DrawBox} from '../editor_base.js';

interface CanvasUndoData {
  type: string
  sliders: number[]
  offsetx: number
  offsety: number
...
export class CanvasOp<
  InputSlots extends PropertySlots = {},
  OutputSlots extends PropertySlots = {},
> extends ToolOp<InputSlots, OutputSlots, ToolContext> {
  drawLines: DrawBox[]
  drawBoxes: DrawBox[]
  _canvasUndo!: CanvasUndoData
  ...
  addDrawBox(x: number, y: number, w: number, h: number): DrawBox {
...
export class ResetOp extends CanvasOp {
  ...
  exec(ctx: ToolContext) {
    ...
    let offsetSliders = pdef.offsetSliders;

    if (!offsetSliders) {
      return;
    }

    const sliderValue = (key: number | string | undefined, fallback: number): number => {
      if (typeof key !== "number") {
        return fallback;
      }
      const def = pdef.sliderDef[key];
      const value = typeof def === "object" ? def.value : undefined;
      return typeof value === "number" ? value : fallback;
    }

    pat.offsetx = sliderValue(offsetSliders.x, 0);
    pat.offsety = sliderValue(offsetSliders.y, 0);
    pat.scale = sliderValue(offsetSliders.scale, 4.75);
  }
}
...
export class CanvasZoomOp extends CanvasOp<{
  offset: Vec2Property
  scale: FloatProperty
  startX: FloatProperty
  startY: FloatProperty
  hasStartMouse: BoolProperty
}> {
  last_mpos: Vector2
  mpos: Vector2
  first: boolean
  ...
  on_mouseup(e: MouseEvent) {
    ...
    if (!cancel) {
      //this.exec(ctx);
      taskManager.range("zoom", 0, steps, (i: number) => {
        let t = i*dt;
