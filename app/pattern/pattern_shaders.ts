import {Curve1D, Matrix4, nstructjs, DataAPI} from '../path.ux/scripts/pathux.js';
import {genBlueMask} from './bluemask.js';
import {Texture} from '../webgl/webgl.js';

/* A uniform value accepted by ShaderProgram.bind. */
export type UniformValue = number | number[] | Texture | Matrix4
export type UniformMap = Record<string, UniformValue>

/* A shader definition as consumed by buildShader / ShaderProgram.fromDef. */
...
export class CurveSet {
  ...
  _regen = true
  size = 0
  tables: (Float32Array | undefined)[]
  gltex: Texture | null = null
  texDimen = 0
  ...
  constructor() {
    ...
    this._regen = true;
    this.size = CurveSet.getDefaultSize();
  ...
  loadJSON(json: Record<'v' | 'r' | 'g' | 'b', Record<string, unknown>>): this {
  ...
  static getShaderCode(name: string, size = this.getDefaultSize()): string {
    let dimen = this.calcTexDimen(size);
    let fdimen = "" + dimen + ".0";
    let idimen = "" + (1.0/dimen);
    let texname = this.texName(name);

    let ch = (ch: number, nvar: string) => {
      return `
      {
...
vec3 ${name}_evaluate(vec3 cinput) {
  ...
  static makeUniforms(name: string, size = this.getDefaultSize()): string {
    return `
      uniform sampler2D ${this.texName(name)};
    `;
  }

  static getName(name: string, ch: string | number): string {
    let names: Record<string | number, string> = {
      'c': 'c', 'r': 'r', 'g': 'g', 'b': 'b',
      0  : 'c', 1: 'r', 2: 'g', 3: 'b'
    };
  ...
  static getEvalCode(name: string, arg: string): string {
    return `${name}_evaluate(${arg})`;
  }

  static apiDefine(api: DataAPI): void {
    let st = api.mapStruct(this, true);

    function onchange(this: {dataref: CurveSet}) {
  ...
  checkTable(): void {
    ...
    for (let i = 0; i < tables.length; i++) {
      let cur = tables[i];
      if (cur === undefined || cur.length !== this.size) {
  ...
  makeTex(gl: AppGL): void {
    this.checkTable();

    if (this.gltex) {
      gl.deleteTexture(this.gltex.texture ?? null);
    }
    ...
    gl.bindTexture(gl.TEXTURE_2D, this.gltex.texture ?? null);
  ...
  static calcTexDimen(size: number): number {
    return Math.pow(2.0, Math.ceil(Math.log(size*4.0)/Math.log(2.0)));
  }

  getTex(gl: AppGL): Texture | null {
    if (!this.gltex) {
      this.makeTex(gl);
    }
  ...
  static texName(name: string): string {
    return name + "_tex";
  }

  setUniforms(gl: AppGL, name: string, uniforms: UniformMap): void {
    this.checkTable();

    let tables = this.tables, size = this.size;

    let tex = this.getTex(gl);
    if (tex) {
      uniforms[CurveSet.texName(name)] = tex;
    }
  }
}
...
export function buildShader(shader: ShaderDef, webgl2?: boolean): ShaderDef {
