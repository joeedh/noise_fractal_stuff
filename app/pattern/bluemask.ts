...
let _bluemasks: Record<number, ImageData> = {};
let _bluetexs: Record<number, Texture> = {};
let loading_masks: Record<number, boolean> = {};

window._bluetexs = _bluetexs;

export function getBlueMaskTex(gl: AppGL, dimen: number): Texture | undefined {
  if (gl.contextBad) {
    return undefined;
  }
...
function bluekey(dimen: number): string {
  return "_bluemask_" + dimen;
}

export function blueMaskValid(dimen: number): boolean {
  return !(dimen in loading_masks);
}

function load_mask_from_ls(dimen: number): void {
...
export function genBlueMask(dimen: number): ImageData | null {
  ...
  for (let i=0; i<totpoint; i++) {
    ...
    if (cells.length > 1) {
      ...
      /* slot is immediately truncated by cells.length-- below */
    }
