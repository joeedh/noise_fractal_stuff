import {EnumProperty, util, nstructjs, Curve1D, PackFlags, ToolProperty, DataAPI, DataStruct, DataPath, DataPathToolProperty, Container, Vector2, Vector3, Vector4} from '../path.ux/scripts/pathux.js';
...
float pattern(float ix, float iy) {
  ...
  static apiDefine(api: DataAPI): DataStruct | undefined {
    ...
    let makeParamGetSet = (def: DataPath, key: string, i: number) => {
      def.customGetSet<Pattern>(function () {
  ...
  _doViewportDraw(ctx: ToolContext, canvas: HTMLCanvasElement, gl: AppGL, enableAccum: boolean,
    ...
    let tilesize = 0, tilew = 0, tileh = 0, tottile = 0, ri = 0;
