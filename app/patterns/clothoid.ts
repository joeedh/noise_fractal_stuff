...
import {
  Vector2, Vector3, nstructjs, util, math, Matrix4, Quat, Vector4, DataAPI, DataStruct, Container
} from '../path.ux/scripts/pathux.js';
import {Preset, savePreset} from '../pattern/preset.js';
import type {UniformMap} from '../pattern/pattern_shaders.js';
import {Sliders} from '../pattern/pattern_types.js';
...
export const ClothoidPresets: Preset[] = [];

let nameBase = 1;

export function add_preset(sliders: number[], opt: PresetOptions = {}): void {
  let pat = new ClothoidPattern();

  for (let k in opt) {
    Reflect.set(pat, k, opt[k]);
  }
...
add_preset(${sliders}, ${optsStr});
  ...
  viewportDraw(ctx: ToolContext, gl: AppGL, uniforms: UniformMap = {}, defines: DefineMap = {}): void {
    let f = isNaN(Math.pow(2.0, sliderNum(this.sliders, "exp")));
  ...
  setup(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    ...
    defines.STEPS = Math.floor(sliderNum(this.sliders, "steps"));
  }
}
