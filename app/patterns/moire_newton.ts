...
import {Pattern, PatternDef, DefineMap} from '../pattern/pattern.js';
...
export const MoireNewtonPresets: Preset[] = [];
...
export function add_preset(
  ...
  for (let k in options) {
    let v = options[k]
    if (k === "func" || k === "newton_mode") {
      if (typeof v === "number") pat[k] = v
    } else if (k === "square_f") {
      pat.square_f = !!v
    } else if (k === "sharpness") {
      if (typeof v === "number") pat.sharpness = v
    }
  }
...
export class MoireNewtonPattern extends Pattern {
  ...
  static apiDefine(api: DataAPI): DataStruct | undefined {
    ...
    let onchange = function (this: {ctx: {pattern: Pattern}}) {
  ...
  copyTo(b: Pattern): void {
    ...
    if (b instanceof MoireNewtonPattern) {
