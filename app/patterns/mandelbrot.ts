...
import {Pattern, PatternFlags, PatternDef, PresetText, DefineMap, toShaderDefines} from '../pattern/pattern.js';
import {savePreset, Preset} from '../pattern/preset.js';
import {ShaderProgram, GPUVertexAttr, RenderBuffer} from '../webgl/webgl.js';
import {taskManager} from '../core/task.js';
import {buildShader, Shaders, UniformMap} from '../pattern/pattern_shaders.js';
import {DataAPI, DataStruct, Container} from '../path.ux/scripts/pathux.js';
import type {ToolContext} from '../core/context.js';

let nameBase = 1;

export const MandelbrotPresets: Preset[] = [];
...
export function add_preset(
  ...
  for (let k in options) {
    let v = options[k]
    if (Reflect.get(pat, k) !== undefined && (typeof v === "number" || typeof v === "boolean")) {
      Reflect.set(pat, k, v)
    }
  }
...
export class MandelbrotPattern extends Pattern {
  _digest: InstanceType<typeof util.HashDigest>
  expandFrame: number
  _showDv: boolean
  cubicInterp: boolean
  _last_drawgen_pointbuf: number | undefined
  regenPointBuf: boolean
  _orbit_mode: boolean
  orbit_seed: number
  orbit_time_step: number
  orbit_t: number
  orbit_t_counter: number
  totpoint: number
  _orbit_update_key: number | undefined
  orbit_accum_shader: ShaderProgram | undefined
  pointbuf_shader: ShaderProgram | undefined
  pointbuf: GPUVertexAttr | undefined
  ...
  get orbit_mode(): boolean {
    return this._orbit_mode;
  }

  set orbit_mode(v: boolean) {
    this.fboCount = v ? 3 : 2;
    this._orbit_mode = v;
  }
  ...
  static apiDefine(api: DataAPI): DataStruct | undefined {
    ...
    let onchange = function (this: {dataref: MandelbrotPattern}) {
  ...
  savePresetText(opts: PresetText = {}, name: string | undefined = undefined): string {
    ...
    return `
add_preset(${this.orbit_mode}, ${this.orbit_seed}, ${sliders}, ${optsStr}${nameStr});
    `.trim();
  }

  setup(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    defines.OFFSET2 = ~~this.sliders.getf("offset2");
  ...
  makePointBuf(gl: AppGL, aspect: number): void {
    ...
    if (!this.pointbuf) {
      this.pointbuf = this.vbo.get(gl, "pointbuf");
    }

    this.totpoint = 0;

    let cos = [];
    let totpoint = ~~(this.sliders.getf("orbpoints")*1000);

    let rand = new util.MersenneRandom(this.orbit_seed);
    let steps = Math.ceil(this.sliders.getf("orbtrail"));

    let offx = this.sliders.getf("x");
    let offy = this.sliders.getf("y");
    let scale = this.sliders.getf("scale");

    const threshold = this.sliders.getf("orbthresh");
  ...
  orbitModeDraw(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    uniforms.alpha = this.sliders.getf("orbalpha");

    if (uniforms.T === 0.0) {
      this.orbit_t = 0.0;
    }

    let speed = /*this.sliders.orbtdist * */ this.sliders.getf("orbspeed")*0.1*this.sliders.getf("scale");
    let noDecay = 1.0;
    ...
    digest.add(this.sliders.getf("orbpoints"));
    digest.add(this.sliders.getf("orbtrail"));
    digest.add(this.sliders.getf("orbthresh"));
    ...
    this.pointbuf_shader.bind(gl, uniforms, toShaderDefines(defines));
  ...
  viewportDraw(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    ...
    if (this.orbit_mode) {
      ...
      let f = this.sliders.getf("orbshift");
      ...
      this.orbit_accum_shader?.bind(gl, uniforms2, toShaderDefines(defines));
  ...
  copyTo(b: Pattern): void {
    ...
    if (b instanceof MandelbrotPattern) {
