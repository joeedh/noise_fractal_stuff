...
export const MountainPresets: Preset[] = [];

let baseNameIdx = 1;

export function add_preset(sliders: number[], opts: PresetOptions): void {
...
add_preset(${JSON.stringify(util.list(this.sliders))}, ${JSON.stringify(opt)});
  ...
  setup(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    let uf = (key: string): string => this.sliders.getUniformRef(key);

    defines.DVFILTER = ~~sliderNum(this.sliders, "dvfilter");
