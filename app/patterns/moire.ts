import {Pattern, PatternFlags, PatternDef, PresetText, DefineMap} from '../pattern/pattern.js';
import {
...
export const MoirePresets: Preset[] = [];
...
export function add_preset(sliders: number[], opt: Record<string, number | boolean> = {}): void {
  ...
  for (let k in opt) {
    let v = opt[k]
    if (k === "moire_func" || k === "band_mode" || k === "smooth_func") {
...
export class MoirePattern extends Pattern {
  ...
  static patternDef(): PatternDef {
    return {
      ...
      sliderDef    : [
        ...
        {name: "exp", value: 1.0, description: "exponent for SUM mode", range: [-2.0, 100.0], speed: 0.5}, //12
  ...
  savePresetText(opt: PresetText = {}): string {
    opt.moire_func = opt.moire_func ?? this.moire_func;
    opt.square_f = opt.square_f ?? this.square_f;
    opt.band_mode = opt.band_mode ?? this.band_mode;
...
add_preset(${sliders}, ${optStr});
  ...
  viewportDraw(ctx: ToolContext, gl: AppGL, uniforms: UniformMap = {}, defines: DefineMap = {}): void {
    let f = isNaN(Math.pow(2.0, this.sliders.getf("exp")));
    this.enableAccum = !isNaN(Number(f)) && isFinite(Number(f));
  ...
  setup(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    ...
    defines.STEPS = Math.floor(this.sliders.getf("steps"));
  }
}
