...
export const NewtonTracePresets: Preset[] = [];

let presetCountBase = 1;

export function add_preset(sliders: number[], options: PresetOptions, fixScale = true, hide = false): void {
  if (hide) {
    presetCountBase++;
    return;
  }
  ...
  for (let k in options) {
    ...
    if (Reflect.get(preset, k) !== undefined) {
      Reflect.set(preset, k, options[k]);
    }
  }
...
export function add_preset_new(sliders: number[], options: PresetOptions, hide = false): void {
...
/* M_PI and cmul() are provided by the shared shader header
 * (pattern_shaders.ts); redefining them here caused a GLSL
 * "macro redefined" / "function already has a body" compile error. */
...
export class NewtonTracePattern extends Pattern {
  ...
  setup(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    ...
    defines.STEPS2 = ~~sliderNum(this.sliders, "steps2");
