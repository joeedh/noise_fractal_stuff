...
import {Pattern, PatternDef, PatternFlags, DefineMap, PresetText} from '../pattern/pattern.js';
import {Preset, savePreset} from '../pattern/preset.js';
import type {UniformMap} from '../pattern/pattern_shaders.js';
import {SliderDef, Sliders} from '../pattern/pattern_types.js';
import type {ToolContext} from '../core/context.js';
import {DataAPI, DataStruct, Container} from '../path.ux/scripts/pathux.js';

type PresetOptions = Record<string, number | boolean>

/* Read a named (dynamically-bound) slider as a number. */
function sliderNum(sliders: Sliders, name: string): number {
  let v = sliders.get(name)?.value
  return typeof v === 'number' ? v : 0
}

export const FractalTracePresets: Preset[] = [];
...
export function add_preset(sliders: number[], opts: PresetOptions = {}, hide = false): void {
...
export class TraceFractalPattern extends Pattern {
  ...
  static patternDef(): PatternDef {
    ...
    let sliderdef: (string | SliderDef)[] = [
      {
        name : "steps",
        type : "int",
        range: [5, 955],
    ...
    for (let i = 0; i < sliderdef.length; i++) {
      ...
      let raw = sliderdef[i];
      let sdef: SliderDef = typeof raw === "string" ? {name: raw} : raw;
  ...
  static apiDefine(api: DataAPI): DataStruct | undefined {
    ...
    let onchange = function (this: {dataref: Pattern}) {
  ...
  setup(ctx: ToolContext, gl: AppGL, uniforms: UniformMap, defines: DefineMap): void {
    ...
    defines.LEVELS = ~~sliderNum(this.sliders, "levels");

    defines.LEVEL_ROT1 = skey("lvlRot1");
    defines.LEVEL_ROT2 = skey("lvlRot2");
    defines.LEVEL_MODE = ~~sliderNum(this.sliders, "lvlMode");
    ...
    defines.STEPS = ~~sliderNum(this.sliders, "steps");
  }
...
add_preset(${sliders}, ${optStr});
  ...
  copyTo(b: Pattern): void {
    ...
    if (b instanceof TraceFractalPattern) {
